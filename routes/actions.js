...
router.get('/', async function (req, res) {
    ...
    if (err) respondWithError(res, 404, "There are no actions yet. Add a new action and try again.")
...
router.get('/:action_id', async function(req, res) {
    ...
    if (err) respondWithError(res, 404, "Couldn't find the action with the specified id.")
...
router.delete('/:action_id', async function (req, res) {
    const [err, action] = await withCatch(actionModel.remove(req.params.id))
